...
class SequenceData(object):
    def __init__(self, seq):
        data_dir = "/home/cs4li/Dev/deep_ekf_vio/results/final_thesis_results/ORB_SLAM2_Mono/data_py2_pickle"
        self.seq = seq
...
def main():
    ...
    seq = "K06"
    # seq = sys.argv[1]
    valid_seqs = ["K04", "K06", "K07", "K08", "K09", "K10"]
    ...
    orb_slam_outputs = np.loadtxt(
            os.path.join("/home/cs4li/Dev/deep_ekf_vio/results/final_thesis_results/ORB_SLAM2_Mono",
    ...
    for i in range(0, len(seq_data.df) - 1):
        ...
        if np.abs(orb_slam_timestamps_adj[orb_slam_meas_idx] - imu_timestamps[0]) < 5e-3:
            ...
            if ROS:
                ...
                t_msg.transform.translation.x = orb_slam_position[orb_slam_meas_idx, 0]
                t_msg.transform.translation.y = orb_slam_position[orb_slam_meas_idx, 1]
                t_msg.transform.translation.z = orb_slam_position[orb_slam_meas_idx, 2]
                t_msg.transform.rotation.x = orb_slam_quat[orb_slam_meas_idx, 0]
                t_msg.transform.rotation.y = orb_slam_quat[orb_slam_meas_idx, 1]
                t_msg.transform.rotation.z = orb_slam_quat[orb_slam_meas_idx, 2]
                t_msg.transform.rotation.w = orb_slam_quat[orb_slam_meas_idx, 3]
        ...
        for j in range(0, len(gyro_measurements) - 1):
            ...
            if ROS:
                ...
                d = rospy.Duration(imu_timestamps[j + 1] - imu_timestamps[j], 0)
                rospy.sleep(d)
            else:
